...
def view_bloco():
    ...
    cont = 0
    select_requests = "select data_do_aviso, nome_paciente, prestador_da_cirurgia,dt_chamada_transf, dt_centro_cirurgico,data_da_cirurgia,dt_entrada_rpa,dt_saida_rpa,centro_Cirurgico from dbamv.vdic_agenda_cirurgia WHERE DT_AVISO >= to_date(SYSDATE) AND DT_AVISO < to_date(SYSDATE+1) AND CENTRO_CIRURGICO = 'BLOCO OBSTETRICO'"
...
def receiver_agenda_obs():
    ...
    if list_agenda:
        ...
        print("Lista bloco obstétrico vazio. O script fará uma nova pesquisa depois de 3 minutos .")

def manda_gravar(list_agenda_cirurgica):
    print(list_agenda_cirurgica)
    arquivo = 'list_agenda_obstetrica.json'
